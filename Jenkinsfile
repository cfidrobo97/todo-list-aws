      ...
      steps {
        withCredentials([string(credentialsId: 'github_pat', variable: 'GITHUB_TOKEN')]) {
          sh '''
            ...
            cd repo
            git clone -b develop https://$GITHUB_TOKEN@github.com/cfidrobo97/todo-list-aws.git .
    ...
    stage('Static Test') {
      steps {
        sh '''
          ...
          ${PY_VENV}/bin/python -m bandit -r src/ -f json -o reports/bandit.json || true
        '''
      }

      post {
        always {
          // Guardar los reportes como artefactos 
    ...
    stage('Rest Test') {
      steps {
        sh '''
          ...
          # Instala deps de test en el mismo venv
        ...
        withCredentials([string(credentialsId: 'github_pat', variable: 'GITHUB_TOKEN')]) {
          sh '''
            ...
            if [ -f CHANGELOG.md ]; then
              sed -i '0,/1\\.0\\.0/s//1.0.1/' CHANGELOG.md
              git add CHANGELOG.md
              git commit -m "chore(release): bump version to 1.0.1" || true
